...
from psy_simple.plugin import ValidateInStrings, ValidateList
from psy_simple.plugin import rcParams as psyps_rc
...
def validate_fit(val):
    def validate(val):
        if isinstance(val, str) and val.startswith("poly"):
            try:
                int(val[4:])
            except ValueError:
                raise ValueError(
                    "Polynomials must be of the form 'poly<deg>' "
                    "(e.g. 'poly3'), not %s!" % val
                )
            else:
                return val
        ...
        return try_and_error(
            validate_callable,
            validate_none,
            ValidateInStrings("fit", ["fit", "linear", "robust"], True),
        )(val)
...
class validate_list(object):
    ...
    def __call__(self, sequence):
        """Validate whether `sequence` is a list with contents of :attr:`dtype`
        ...
        try:
            if self.dtype is None:
                sequence = self.listtype(sequence)
            else:
                sequence = self.listtype(map(self.dtype, sequence))
        except TypeError:
            if self.dtype is None:
        ...
        if self.length is not None and len(sequence) != self.length:
            raise ValueError(
...
def validate_p0(value):
    ...
    for i, v in enumerate(value):
        value[i] = try_and_error(
            ValidateList(float), ValidateInStrings("p0", ["auto"], True)
        )(v)
    return value
...
rcParams = RcParams(
    defaultParams={
        ...
        "project.plotters": [
            {
                ...
                "densityreg": {
                    ...
                    "default_slice": None,
                    "summary": (
                        "Make a density plot and draw a fit from x to y of "
                        "points"
                    ),
                },
            },
        ...
        "plotter.linreg.xrange": [
            "minmax",
            validate_limits,
            "The fit limits of the line plot",
        ],
        ...
        "plotter.linreg.id_color": [
            None,
            psyps_rc.validate["plotter.simple.color"],
        ...
        "plotter.linreg.bootstrap.random_seed": [
            None,
            try_and_error(validate_none, validate_int),
